...
The grid component helps position and align content, creating a conistent layout. A grid can be used on the page overall, within page sections,
...
the layout as the browser widthe changes.
...
If you use a symbol grid make sure that it is visable before uploading to InVision
...
The prop gridTemplateColumns can be used to easily create layouts that span various amounts of columns.
...
Some of the cells also use the prop `justifySelf` to align that cell's content to the center of the cell along the x axis.
...
The Grid prop `gridTemplateArea` can be combined with Cell components using the `gridArea` prop to easily set a layout with varying column widths.
