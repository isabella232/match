...
There are currently 3 supported icon packages. `@twilio-labs/match-icons-twilio`, `@twilio-labs/match-icons-sendgrid`, and the universal `@twilio-labs/match-icons-product`. Icons have an additional peer dependency of `react-uid`.

```shell
yarn add @twilio-labs/match-icons-twilio @twilio-labs/match-icons-product react-uid
```
...
The default icon color may be altered using a text color token.
