...
Individual token names are formatted in camelcase using the concatenated group and singular names. For instance, `theme.breakpointSmall` or `theme.colorBaseBlue`.

Additionally, tokens or grouped into collections that may be accessed via dot notation. For instance, `theme.breakpoints.small` or `theme.colors.baseBlue`.
...
- JavaScript: `breakpointSmall` or `breakpoints.small`
