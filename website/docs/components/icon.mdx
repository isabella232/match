...
There are currently 3 supported icon packages. `@twilio-labs/match-icons-twilio`, `@twilio-labs/match-icons-sendgrid`, and the universal `@twilio-labs/match-icons-product`. These may be installed individually or bundled together as `@twilio-labs/match-icons`.

```shell
yarn add @twilio-labs/match-icons
```
