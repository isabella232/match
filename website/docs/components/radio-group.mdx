...
**Sources:** [Github](https://github.com/twilio-labs/match/tree/main/packages/match-components/src/radio-group)
...
Make sure all Radios use the same `name`.

<VirtueCrime>
  <Virtue>
    Use radio buttons in a radio group with at least two or more choices - where
    a user must select one option only.
  </Virtue>
  <Crime>Don't use a radio button on its own. </Crime>
  <Virtue>
...
A required Radio Group will automatically check to make sure one of the options is selected upon submitting. This can be overridden using the `validate` prop. Validation can be turned off using the `noValidate` prop.
